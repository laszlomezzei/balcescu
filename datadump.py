...
def get_kinds():
    kinds = ["Company", "Canvas", "Fixture", "Product"]
    return kinds
...
def GetSchema(_app=None):
    ...
    """
    req = datastore_admin.api_base_pb.StringProto()
    req.set_value(datastore_admin.datastore_types.ResolveAppId(_app))
    resp = datastore_admin.datastore_pb.Schema()

    datastore_admin._Call('GetSchema', req, resp)
    ...
    def get(self):
        ...
        for kind in get_kinds():
            ...
            for entity in entities:
                counter = 0
                self.response.out.write("insert into " + kind.lower() + " (" )
                ...
                for key, value in entity.items():
                    if type(value) != type(db.Key()) and value:
                        ...
                        if type(value) == type(u'') or type(value) == type(""):
                            self.response.out.write("'")
                            self.response.out.write(value.replace("'","\\'"))
                            self.response.out.write("'")
                        else:
